...
//				 booting information.  
// 
//  PARAMETERS: bootSeconds: number of seconds to delay.
//****************************************************************************
void WinUtils::delayBoot(int bootSeconds)
{
	...
	while (!bootComplete)
	{
		if ((time(nullptr) - seconds1) >= bootSeconds)
		{
			bootComplete = true;
		}
		if ((time(nullptr) - seconds2) >= 1)
		{
			seconds2 = time(nullptr);
			for (int i = 0; i < 8; ++i)
			{
				std::cout << "\r" << clk[idx++] << " (" << (bootSeconds + 1) -
					(time(nullptr) - seconds1) << " secs.)   ";
				if (idx > 3)
				{
					idx = 0;
				}
...
void WinUtils::wait(int seconds)
{
	...
	while (seconds)
	{
		if ((time(nullptr) - currentSeconds) >= 1)
		{
			std::cout << "\r" << seconds - 1 << " secs...";
			seconds = seconds - 1;
			currentSeconds = time(nullptr);
		}
	}
	std::cout << std::endl;
}
...
// 
//  PARAMETERS: prompt: text for the user prompt.
//****************************************************************************
